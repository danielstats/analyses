...
### Numerical summaries
It becomes immediately apparent that the two fountains have very different statistical parameters. Note that the mean fill time of the bottle filler fountain is 49.6, while the regular fountain has a much lower mean of 30.77. See below table for more measures of central tendency, the mode for both fountains is irrelevant due to having no repeated values.
...
Note that the standard deviations of the two fountains are 0.758 and 1.203 respectively. There is clearly a very tight spread for both fountains, which bodes well for our testing as it indicates little variance inside groups. Examining the difference between groups, we will calculate some measures of spread: 
...
![](fountain_files/figure-html/main_plot-1.png)<!-- -->
...
<img src="fountain_files/figure-html/test_reqs-1.png" width="50%" />
...
In the results below, see that the test statistic is 32.44, which is quite high with our degrees of freedom. If there is statistically significant evidence that the two fountains have different actual fill times, we will see a p-value less than $0.05$. 

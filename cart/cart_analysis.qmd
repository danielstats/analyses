...
We will additionally calculate Cronbach's $\alpha$, having removed the "Try scale" items (which will be addressed later) to ensure unidimensionality(hopefully):
...
As we will see later, there are several items who correlate so poorly that more of the BYU students than the Rexburgians answered correctly. The Alpha calculation indicates several items we might remove. If we take out the ones that it indicates, we are left with the following, drastically shorter test:
...
as we can see, the $\alpha$ and $\lambda$ coefficients are much higher, despite a shorter test.
